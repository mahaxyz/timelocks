...
task(`transfer-timelock-admin`).setAction(async (params, hre: HardhatRuntimeEnvironment) => {
  ...
  const timelock = await hre.ethers.getContractAt('MAHATimelockController', timelockD.address);
  ...
  await waitForTx(await timelock.revokeRole(await timelock.DEFAULT_ADMIN_ROLE(), deployer.address));
});
